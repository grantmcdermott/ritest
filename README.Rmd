...
routine (by 
[Simon Heß](https://github.com/simonheb)) for conducting 
[**randomization inference**](https://dimewiki.worldbank.org/Randomization_Inference). 
...
[`Installation`](#Installation) | [`Examples`](#Examples) ([`toy data`](#toy-data), [`real-life data`](#real-life-data)) | [`Benchmarks`](#Benchmarks)
...
I'll also load the **fixest** and **modelsummary** packages to help 
demonstrate some additional functionality in the examples that follow.
...
- `h0` is the sharp null hypothesis that you want to test (defaults to '=0',
i.e. a standard two-sided test of `resampvar` against a null value of zero.)
...
Similarly, support for regression tables is enabled via **ritest's** compatability
with `modelsummary::msummary()`. I recommend displaying p-values instead of the 
default standard errors. This is particularly important when comparing against
a parametric model, as I do below.
...
Next, we'll replicate an example from David McKenzie's nice 
...
randomized control trial about supply chains in Colombia.
You can download the original .dta file from David's website 
[here](https://drive.google.com/open?id=0B9C9RwWKZrUNazdyVXFkSTlTNGc). 
Alternatively, this implementation bundles the dataset as part of the R package 
(see: `?colombia`). So you can also access and export it easily that way too.
...
One nice feature of this R implementation is that it should complete very 
quickly. This time, the 5,000 simulations only take around **6 seconds**.
