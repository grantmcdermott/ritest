...
We can also visualize the result using the default `plot` method. This
...
fixed-effects in our regression model. Again, we obtain an RI ejection
rate (0.003) that is very similar to the parametric p-value (0.004) from
...
randomized control trial about supply chains, which was conducted in
Columbia. You can download the data from David’s website
...
blog post for more on that.) However, the essential thing to know is
that I’m going to run 5,000 RI permutations on a pretty standard
...
co_ri
#> 
#> Call: feols(fml = dayscorab ~ b_treat + b_dayscorab + miss_b_dayscorab + round2 + round3 | b_pair, data = co, cluster = ~b_block)
#> Res. var(s): b_treat
#> Strata var(s): b_pair
#> Strata: 31
